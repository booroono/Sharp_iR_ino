...
// 임계값
#define PS_HTH 155  // high threshold (800 -> 155)
#define PS_LTH 125  // low threshold (600 -> 125)
...
void init_device(struct gp2ap_data* p_gp) {
    ...
    wdata = 0xB0; gp2ap_i2c_write(0x86, &wdata, p_gp); // Pgain = 0x86, 0xB0 (원래 게인값)
    ...
    wdata = 0x61; gp2ap_i2c_write(0xC1, &wdata, p_gp); // 원래 오프셋값 (0x41 -> 0x61)
    wdata = 0x00; gp2ap_i2c_write(0xC2, &wdata, p_gp);
    wdata = 0x30; gp2ap_i2c_write(0xC5, &wdata, p_gp); // Gain_F = 0xC5, 0x30 (원래 필터 게인값)
    wdata = 0x24; gp2ap_i2c_write(0xD0, &wdata, p_gp);
    wdata = 0x3C; gp2ap_i2c_write(0xD1, &wdata, p_gp); // IIR_THD = 0xD1, 0x3C (60)
    wdata = 0x09; gp2ap_i2c_write(0xF0, &wdata, p_gp); // PLDrive = 0xF0, 0x09 (원래 LED 전류값)
