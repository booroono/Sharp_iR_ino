...
void setup() {
    ...
    // open 값을 낮추기 위한 설정 조정 (목표 약 400-500)
    uint8_t wdata;
    Tca_Select(0); // 멀티플렉서 채널 0 선택
    
    // 표준 설정 레지스터 (감도 크게 감소)
    wdata = 0x08; gp2ap_i2c_write(0x84, &wdata, &ir_sensor); // PWTIME = 0x84, 0x08 (16ms)
    wdata = 0x93; gp2ap_i2c_write(0x85, &wdata, &ir_sensor); // Ppulse = 0x85, 0x93 (3ea)
    wdata = 0x50; gp2ap_i2c_write(0x86, &wdata, &ir_sensor); // Pgain 감소 (0xB0 -> 0x50) - 게인 크게 감소
    wdata = 0x10; gp2ap_i2c_write(0xC5, &wdata, &ir_sensor); // Gain_F 감소 (0x30 -> 0x10) - 필터 게인 크게 감소
    
    // 추가 설정 레지스터
    wdata = 0x05; gp2ap_i2c_write(0xF0, &wdata, &ir_sensor); // PLDrive 감소 (0x09 -> 0x05) - LED 전류 감소
    wdata = 0x3C; gp2ap_i2c_write(0xD1, &wdata, &ir_sensor); // IIR_THD = 0xD1, 0x3C (60)
    wdata = 0x41; gp2ap_i2c_write(0xC1, &wdata, &ir_sensor); // 오프셋 감소 (0x61 -> 0x41) - 기준점 낮게 조정
}
